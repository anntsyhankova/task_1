...
public class SimpleFTPClient {
    ...
    public synchronized List<String> list() throws IOException{
        ...
        Socket dataSocket = new Socket(ip, port);
        BufferedReader dataOutput = new BufferedReader(new InputStreamReader(dataSocket.getInputStream()));
        System.out.println(dataOutput.readLine());
        return null;
    }

    public synchronized boolean port(String url) throws IOException{
        sendLine("PORT "+url);
        String response = readLine();
        System.out.println(response);
        return (response.startsWith("200 "));
